...
public class KinhnghiemDao {
    ...
    public void DeleteKn(String id)
    {
    	String query = "DELETE FROM `doan`.`kinhnghiem` WHERE (`idkn` = ?)";
    	try {
			con = DbCon.getConnection();
			stm = con.prepareStatement(query);
			stm.setString(1, id);
    ...
    public void InsertKn(String nameda, String kythuat, String mota,String namkn, String idcv)
    {
    	String query = "INSERT INTO `doan`.`kinhnghiem` (`nameda`, `kythuat`, `mota`,`namkn`, `idcv`) VALUES (?, ?, ?,?, ?)";
    	try {
			con = DbCon.getConnection();
			stm = con.prepareStatement(query);
			stm.setString(1, nameda);
